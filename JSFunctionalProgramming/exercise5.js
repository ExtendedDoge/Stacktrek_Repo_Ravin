...
const result1 = userList.map((user) => {
    return user.fullName.replace('_', ' ').toUpperCase(user);
})
console.log(result1)
    //
const result2 = userList.map((money) => {
        return money.salary
    })
    //
const result3 = result2.reduce((total, num) => {
    return total + num
}, 0)
console.log(result3)
    //
const lesserValue = result2.filter((checkLessThan10k) => {
    return checkLessThan10k < 10000;
})
const greaterValue = result2.filter((checkMoreThan10k) => {
    return checkMoreThan10k > 10000;
})
let myPromise = new Promise((resolve, reject) => {
    if (lesserValue == lesserValue) {
        resolve('True')
    } else {
        reject('False')
    }
...
myPromise
    .then(function() {
        console.log("List of Salaries with less than 10,000: " + lesserValue);
    })
    .catch(function() {
        console.log("List of Salaries with more than 10,000: " + greaterValue);
    });